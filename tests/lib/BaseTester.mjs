...
const  lazyImport = async (path) =>{
   const newPath = pathToFileURL(path).href
    if (path.slice(-4) === 'json'){
        return   import(newPath, {assert: {type: 'json'}})
    }else {
...
class BaseTester {
    ...
    #lockDBExclusively = async () => {
        ...
        await this.sequelize.query(`SELECT * FROM "SequelizeMeta" FOR UPDATE`, { type: QueryTypes.SELECT, lock: true })
    }
}
