...
class UseCasesGenerator extends Base {
    constructor({ model, config, logger }) {
        ...
        this.folders = {
            templates : path.posix.join(config.templatesFolder, 'use-cases'),
        ...
        this.files = {
            ...
            usecasesBase : path.posix.join(config.usecasesFolder, 'Base.mjs'),
            model        : path.posix.join(config.modelsFolder, `${this.modelNames.original}.mjs`),
            dmx          : path.posix.join(config.modelsFolder, 'X.mjs')
        }
    }
    ...
    #generateUseCases = async ({ override, actions } = {}) => {
        ...
        for (const templateFile of templatesList) {
            ...
            const fileString = this.fillTemplateWithData(template, {
                ...
                // Paths
                'PACKAGES_RELATIVE_PATH'     : path.posix.relative(this.folders.usecases, this.files.packages),
                'USECASE_BASE_RELATIVE_PATH' : path.posix.relative(this.folders.usecases, this.files.usecasesBase),
                'DUMPS_RELATIVE_PATH'        : path.posix.relative(this.folders.usecases, this.files.dumps),
                'MODEL_RELATIVE_PATH'        : path.posix.relative(this.folders.usecases, this.files.model),
                'DMX_RELATIVE_PATH'          : path.posix.relative(this.folders.usecases, this.files.dmx)
            })
